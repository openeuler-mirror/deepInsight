...
```
3. 命令行使用
- 查看帮助：`di --help` 或 `python -m deepinsight.cli.main --help`
...
- 深度研究助手（resch）
  - 启动研究：`di resch gen --topic "人工智能发展趋势"`
  - 查看帮助：`di resch --help`

- 后端服务（api）
  - 启动后端服务：`di api start --config ./config.yaml`
  - 指定专家配置（可选）：`di api start --config ./config.yaml --expert-config ./experts.yaml`
  - 也可通过环境变量指定：`DEEPINSIGHT_CONFIG=./config.yaml di api start`
