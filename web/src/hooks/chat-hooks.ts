...
// 获取ask
export const useGetAsk = (pureFetch = false) => {
  const {
    data,
    isFetching: loading,
    refetch,
    ...
    queryFn: async (...params) => {
      ...
      const { data } = await chatService.getAsk();
...
export const useFetchResearchDialog = (pureFetch = false) => {
  // const {
  //   data,
  //   isFetching: loading,
  //   refetch,
  // } = useQuery<IDialog>({
  //   queryKey: ['fetchResearchDialog'],
  //   initialData: {} as IDialog,
  //   gcTime: 0,
  //   refetchOnWindowFocus: false,
  //   queryFn: async (...params) => {
  //     console.log('🚀 ~ queryFn: ~ params:', params);
  //     const { data } = await chatService.getResearchDialog();
  ...
  const {
    ...
    mutationFn: async (conversationIds: string[]) => {
      const { data } = await chatService.removeConversation({
        conversationList: conversationIds,
      });
      if (data.code === 0) {
        queryClient.invalidateQueries({ queryKey: ['fetchConversationList'] });
      }
