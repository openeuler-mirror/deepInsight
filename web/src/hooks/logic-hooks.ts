...
export const useSendMessageWithSse = (
  ...
  const send = useCallback(
    async (
      ...
      addStreamAnswer?: (answer: IAnswer) => void,
...
export const useGeneratePptSse = (
  url: string = api.generatePptConversationSSE,
  addMessageFiles?: (file: IFile) => void,
) => {
  const [answer, setAnswer] = useState<IFile>({} as IFile);
  const [done, setDone] = useState(true);
  const sseRef = useRef<AbortController>();
  ...
  //   }
  //   timer.current = setTimeout(() => {
  //     setAnswer({} as DeepInsightMessage);
  //     clearTimeout(timer.current);
  //   }, 1000);
  // }, []);

  const generatePptRequest = useCallback(
    async (
      body: any,
      controller?: AbortController,
    ): Promise<{ response: Response; data: ResponseType } | undefined> => {
    ...
    [initializeSseRef, url, setDone, setAnswer],
  );
  ...
  return { generatePptRequest, answer, done, setDone, stopOutputMessage };
};
...
export const useSelectDerivedMessages = () => {
  ...
  // Add the streaming message deduplicate by message ID in the message list
  const addNewestAnswerDeduplicateByMessageId = useCallback(
    (answer: IAnswer) => {
      setDerivedMessages((pre) => {
        var last_id = pre?.at(-1)?.id;
        if (last_id === answer.id) {
          return [
            ...(pre?.slice(0, -1) ?? []),
            {
              role: MessageType.Assistant,
