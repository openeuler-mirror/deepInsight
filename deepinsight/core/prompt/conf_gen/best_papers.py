...
2. 获取论文相关图片并准备嵌入  
3. 获取并分析论文内容  
4. 生成结构化分析报告（图片嵌入语义位置）（报告中的关键洞察内容需要调用 summary-agent 获取）  
5. 保存文件到指定目录并反馈完整路径
...
8. **关键洞察（专家视角总结）**：
   总结内容需要调用  summary-agent 生成，生成结果中的观点按照序号顺序，放在原文总结位置即可，其他多余的废话剔除。输入给 summary-agent的上下文内容必须包括1、主题与作者信息（200字）2、问题与挑战（300字），3、关键技术及技术效果（600字）
...
使用 Markdown 格式输出以下部分，图片嵌入在相关章节中（无独立“图片与图表”章节）：
 ...
 5. 技术效果：
    ...
    - 使用项目符号列出主要性能提升
 6. 关键洞察：
    调用 summary-agent 获取，保留summary-agent 返回的有价值的多条关键洞察观点，观点以外的废话无需保留，直接剔除
